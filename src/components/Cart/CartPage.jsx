...
import { db, doc, collection, setDoc, deleteDoc, getDocs ,getDoc} from '../firebase';
...
<ol className="relative flex justify-between items-center space-x-4 text-sm font-medium text-gray-500 dark:text-gray-400">
  {steps.map((stepData, index) => (
    <li
      key={index}
      className={`flex items-center ${index <= step ? 'text-green-800 dark:text-green-500' : 'text-gray-400'}`}
    >
      <span
              ...
              <div className="lg:col-span-3">
                {cartItems.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {cartItems.map((item) => (
                      <div key={item.id} className="p-4 bg-white dark:bg-gray-800 shadow-md rounded-lg">
                        <img src={item.img} alt={item.title} className="h-60 object-cover rounded" />
  ...
  ><FontAwesomeIcon icon={faTimes} className="h-5 w-5" />
            ...
            <p className="mb-4">Do you want to remove this item or add it to your wishlist?</p>
...
<button
  ...
  className={`py-2 px-4 text-white rounded flex items-center ${
    isWishlist ? 'bg-red-500 hover:bg-red-600' : 'bg-gray-600 hover:bg-gray-700'
  }`}
>
  <FontAwesomeIcon
    icon={faHeart}
    className={`mr-2 ${isWishlist ? 'text-red-500' : 'text-white'}`}
  />
  {isWishlist ? 'Remove from Wishlist' : 'Add to Wishlist'}
            ...
            <div className="mt-4 flex justify-end">
                  ...
                  />
                    <p className="text-gray-700 dark:text-gray-300 text-xl animate-pulse">Your Cart is Empty.</p>
                </div>
                )}
              </div>
